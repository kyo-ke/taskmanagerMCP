...
func registerTaskCreate(s *server.MCPServer, client *taskmanager.Client) {
	tool := mcp.NewTool("task_create",
		mcp.WithDescription("Create a task in taskmanager"),
		mcp.WithNumber("priority", mcp.Required(), mcp.Description("Priority (1..)")),
	...
	s.AddTool(tool, func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		out, err := client.CreateTask(ctx, taskmanager.Task{
